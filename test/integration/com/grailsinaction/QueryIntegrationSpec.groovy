...
class QueryIntegrationSpec extends Specification {
    ...
    void "Simple property comparison" () {
        ...
        users*.loginId == ["frankie", "pansy"]
    }

    void "Multiple criteria" () {
        when: "A user is selected by loginId or password"
        def query = User.where {loginId == "frankie" || password == "crikey"}
        def users = query.list(sort: "loginId")
    ...
    */
    ...
    void "Query on association" () {
        ...
        then: "Alist of the followers of the given user is returned"
        users*.loginId == ["pansy", "phil"]
    }
